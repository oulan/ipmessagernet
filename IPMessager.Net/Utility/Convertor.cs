...
namespace IPMessagerNet.Utility
{
	public static class Convertor
	{
		...
		public static string ToJsonInfoWithProgress(this FileTaskItem item)
		{
			...
			sb.Append("{");
			sb.AppendFormat("pkgid:\"{10}\",index:{0},curname:\"{11}\",filename:\"{1}\",filecount:{2},sended:{3},filesize:\"{4}\",sizesended:\"{5}\",percentage:{6},timeused:\"{7}\",timerest:\"{8}\",state:{9},speed:'{11}/S'",
				item.Index, Helper.ConvertJsString(item.Name), item.FileCount, item.FinishedFileCount, item.TotalSize.ToSizeDescription(),
				item.FinishedSize.ToSizeDescription(), percentage, string.Format("{0:00}:{1:00}:{2:00}", usedTime.Hours, usedTime.Minutes, usedTime.Seconds), string.Format("{0:00}:{1:00}:{2:00}", restTime.Hours, restTime.Minutes, restTime.Seconds), (int)item.State, item.TaskInfo.PackageID, ((ulong)speed).ToSizeDescription(),
		...
		public static string ToJsonInfo(this FileTaskInfo task)
		{
			System.Text.StringBuilder sb = new StringBuilder();
			sb.AppendLine("{");
			sb.AppendFormat("pkgid:'{0}',host:'{1}',", task.PackageID, task.RemoteHost.HostSub.Ipv4Address.ToString());
			sb.AppendLine("tasks:[");
